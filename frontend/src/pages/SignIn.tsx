...
function SignIn() {
  ...
  const [regData, setRegData] = useState<IAuth>({ email: "", password: "", confirm_password: "" });
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (regData.password === regData.confirm_password) {
      const res = await signin({ email: regData.email, password: regData.password });
      setToken(res.access_token);
      setDecodedToken(jwtDecode(res.access_token));
      navigate("/");
    }
    else {
      setShowAlert(true);
